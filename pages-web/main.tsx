...
const initialLanguage=normalizeLanguage(new URLSearchParams(window.location.search).get('lang'));
createRoot(document.getElementById('root')!).render(admin
  ? <Suspense fallback={<p role="status">Cargando acceso seguro…</p>}><Admin area={path.split('/')[2]||'resumen'}/></Suspense>
  : path==='/guia'?<PublicGuide initialLanguage={initialLanguage}/>:<Cover initialLanguage={initialLanguage}/>);
