...
export default function Home(){
  ...
  return <main className="app-shell" dir={isRtl?'rtl':'ltr'}>
    <header className="app-header">
      ...
      <select value={language} onChange={e=>setLanguage(e.target.value as LanguageCode)} aria-label="Language">{languageCodes.map(code=><option key={code} value={code}>{ui[code].language}</option>)}</select>
    </header>
    ...
    {screen==='home'&&<section className="screen home-screen">
      ...
      <button className="provisional-menu-tab" onClick={()=>navigate('menu')}><span>♨</span><div><small>{copy.provisional}</small><strong>{menu.title}</strong><p>{menu.reference}</p></div><i>›</i></button>
      <button className="primary-action" onClick={()=>navigate('registration')}>{text.registration} →</button>
    </section>}
    ...
    {screen==='program'&&<section className="screen">
      ...
      <ol className="program-list">
        {copy.schedule.map(([time,title,detail],index)=><li key={`${time}-${title}`}><time>{time}</time><div><strong>{title}</strong>{detail&&<span>{detail}</span>}
      ...
      <button className="primary-action" onClick={()=>navigate('map')}>{text.seeMap} →</button>
    </section>}
    ...
    {screen==='map'&&<section className="screen map-screen">
      ...
      <ol className="map-legend">{mapPins.map(pin=>{const card=cards.find(c=>c.id===pin.id)!;return <li key={pin.id}><button onClick={()=>navigate(`card-${pin.id}`)}><b>{pin.number}</b><span>{card.title}</span><i>›</i></button></li>})}</ol>
      <a className="primary-action link-button" href={FULL_ROUTE_URL} target="_blank" rel="noreferrer">{text.mapFull}<External/></a>
      <p className="map-credit">Mapa: <a href={mapAttribution.commons} target="_blank" rel="noreferrer">{mapAttribution.credit}</a> · {mapAttribution.license}</p>
    ...
    {screen==='visit'&&<section className="screen guide-screen">
      ...
      <div className="guide-list">{cards.map(card=><button className="guide-card" key={card.id} onClick={()=>navigate(`card-${card.id}`)}><img src={imageUrl(card.detail.image)} alt=""/><div><small>{card.detail.facts[0][1]}</small><h2>{card.title}</h2><p>{language==='es'?card.detail.short:`${card.body.slice(0,145)}…`}</p><b>{text.readMore} →</b></div></button>)}</div>
    </section>}

    {current&&<article className="detail-screen">
      <div className="detail-photo"><img src={imageUrl(current.detail.image)} alt={current.detail.alt}/><div className="photo-caption">{current.detail.credit} · {current.detail.license}</div></div>
      <div className="screen detail-body"><p className="eyebrow">{text.guide}</p><h1>{current.title}</h1><p className="detail-lead">{current.body}</p>
        <section className="facts"><h2>{text.highlights}</h2><dl>{current.detail.facts.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section>
        ...
        <section className="audio-panel"><div><span>▶</span><div><small>{text.audio}</small><strong>{current.title}</strong></div></div><audio controls preload="none" src={imageUrl(`audio/${language}/${current.id}.wav`)} aria-label={`${copy.listen}: ${current.title}`}/></section>
