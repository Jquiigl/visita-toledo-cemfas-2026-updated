...
function AnalysisAssistant({kind, ready, hasData}: {kind: AnalysisKind; ready: boolean; hasData: boolean}) {
  ...
  return <article className="admin-card admin-ai">
    ...
    {result && <section className="analysis-result" aria-live="polite">
      <h3>Interpretación mediante IA · borrador</h3><p className="admin-muted">{new Date(result.generatedAt).toLocaleString('es-ES')} · {result.model} · {result.source === 'mock' ? 'DATOS FICTICIOS' : result.source === 'google' ? 'Datos de Google Sheets' : 'Datos de Supabase'}</p>
...
export function SurveyPanel({data, onPrint}: {data: Data; onPrint: (reports: Report[]) => void}) {
  ...
  const extra = data.extraSurvey;
  const extrasReport: Report = {title: 'Encuesta · escalas adicionales', headers: ['Pregunta', 'Respuesta o indicador', 'Total o valor', 'Base válida'], rows: [
    ['Relación entre alumnos y padrinos (0–10)', 'Media / 10', extra.relationship.mean?.toFixed(2) ?? '—', String(extra.relationship.n)],
    ['Relación entre alumnos y padrinos (0–10)', 'Sin respuesta', String(extra.relationship.missing), String(extra.relationship.n)],
    ['Relación entre alumnos y padrinos (0–10)', 'No válidas', String(extra.relationship.invalid), String(extra.relationship.n)],
    ...extra.relationship.distribution.map(d=>['Relación entre alumnos y padrinos (0–10)', String(d.score), String(d.count), String(extra.relationship.n)]),
    ...extra.categories.flatMap(q=>[...q.distribution.map(d=>[q.question, d.label, String(d.count), String(q.n)]),[q.question,'Sin respuesta',String(q.missing),String(q.n)],[q.question,'No válidas',String(q.invalid),String(q.n)]]),
  ]};
  ...
  return <>
    ...
    {extra.available&&<><article className="admin-card"><h2>Relación entre alumnos y padrinos · escala 0–10</h2><p>Media: {extra.relationship.mean?.toFixed(2)??'—'} / 10 · N={extra.relationship.n}. Esta pregunta no se mezcla con las medias sobre 5.</p>{extra.relationship.distribution.map(d=><div className="admin-bar" key={d.score}><span>{d.score}</span><meter min={0} max={Math.max(1,extra.relationship.n)} value={d.count} aria-label={`${d.score} puntos: ${d.count} respuestas`}/><small>{d.count}</small></div>)}<p>Sin respuesta: {extra.relationship.missing} · No válidas: {extra.relationship.invalid}</p></article><div className="admin-two">{extra.categories.map(q=><article className="admin-card" key={q.question}><h2>{q.question}</h2><p>N válido: {q.n} · Sin respuesta: {q.missing} · No válidas: {q.invalid}</p>{q.multiple&&<p>Selección múltiple: los porcentajes pueden sumar más del 100 %. No es una medida NPS.</p>}{q.distribution.map(d=><div className="survey-choice" key={d.label}><span>{d.label}</span><meter min={0} max={100} value={d.percent} aria-label={`${d.label}: ${d.count} respuestas`}/><small>{d.count} · {d.percent.toFixed(1)}%</small></div>)}</article>)}</div><div className="admin-actions"><button onClick={()=>saveText('toledo-encuesta-escalas-adicionales.csv',safeCsv([extrasReport.headers,...extrasReport.rows]),'text/csv;charset=utf-8')}>Exportar escalas adicionales CSV</button><button onClick={()=>onPrint([report,extrasReport])}>Todas las escalas / PDF</button></div></>}
